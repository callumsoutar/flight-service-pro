...
export function getNextMembershipYearStart(
  ...
  if (referenceDate > endDate) {
    ...
    return new Date(nextYear, membershipYearConfig.start_month - 1, membershipYearConfig.start_day);
  }
 ...
 */
export function formatMembershipYear(
  membershipYearConfig: MembershipYearConfig
): string {
  const { start_month, start_day, end_month, end_day, description } = membershipYearConfig;
  
  if (description) {
    return description;
  }
  
  const startMonthName = new Date(2024, start_month - 1, 1).toLocaleString('default', { month: 'long' });
  const endMonthName = new Date(2024, end_month - 1, 1).toLocaleString('default', { month: 'long' });
...
export function validateMembershipYearConfig(config: MembershipYearConfig): {
  ...
  if (startDate.getMonth() !== config.start_month - 1 || startDate.getDate() !== config.start_day) {
    return { isValid: false, error: 'Invalid start date (day does not exist in the specified month)' };
  }
  
  if (endDate.getMonth() !== config.end_month - 1 || endDate.getDate() !== config.end_day) {
    return { isValid: false, error: 'Invalid end date (day does not exist in the specified month)' };
  }

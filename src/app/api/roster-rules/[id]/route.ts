...
export async function GET(
  ...
  try {
    ...
    // STEP 2: Authorization - Only instructors+ can view roster rules
...
export async function PATCH(
  ...
  try {
    ...
    // STEP 2: Authorization - Only admin/owner can update roster rules
    const { data: userRole, error: roleError } = await supabase.rpc('get_user_role', {
      user_id: user.id
    });
    ...
    if (!userRole || !['admin', 'owner'].includes(userRole)) {
      return NextResponse.json({
        error: 'Forbidden: Updating roster rules requires admin or owner role'
      }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    // STEP 2: Authorization - Only admin/owner can delete roster rules
    const { data: userRole, error: roleError } = await supabase.rpc('get_user_role', {
      user_id: user.id
    });
    ...
    if (!userRole || !['admin', 'owner'].includes(userRole)) {
      return NextResponse.json({
        error: 'Forbidden: Deleting roster rules requires admin or owner role'
      }, { status: 403 });
    }

        ...
        const instructorsWithEndorsements = await Promise.all(
          (instructorsData.instructors || [])
            ...
            .map(async (instructor: {
              ...
              try {
                // Get endorsement names from boolean columns
                const endorsementNames = [];
                if (instructor.night_removal) endorsementNames.push('Night Removal');
                if (instructor.aerobatics_removal) endorsementNames.push('Aerobatics Removal');
                if (instructor.multi_removal) endorsementNames.push('Multi Removal');
                if (instructor.tawa_removal) endorsementNames.push('TAWA Removal');
                if (instructor.ifr_removal) endorsementNames.push('IFR Removal');
